...
class RankService {
  ...
  getPlayerRank(
    ...
    const ranks = playedDates.map(date => {
      ...
      const index = calculateRank.findIndex(([p]) => p.Id === player.Id)
  ...
  private calculateRank(
    ...
    players.map(player =>
      calucMap.set(player, player.getScoreByDate(date).point),
    )
  ...
  private sortCalcMaps(calucMap: Map<Player, Point>) {
    const result = [...calucMap.entries()].sort(
      (a, b) => b[1].getPoint() - a[1].getPoint(),
    )
    return result
  }
}
